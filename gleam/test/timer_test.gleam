...
// Smoke tests: scheduling then immediately clearing must not crash (and must not
...
pub fn request_and_cancel_animation_frame_test() {
  let id = timer.request_animation_frame(fn(_timestamp) { Nil })
  timer.cancel_animation_frame(id)
}
